...
module.exports = (argv=[]) ->
  ...
  options.usage """
    Usage: cson-sort [options] cson_file --output cson_file
           cson-sort [options] < cson_file [> cson_file]

    Sanitizes CSON.

    If no input file is specified then the CSON is read from standard in.

    If no output file is specified then the CSON is written to standard out.
  """
  ...
  parseData = (data) ->
    ...
    cleaned = stringify(object, undefined, 2) + "\n"
    if argv.output
